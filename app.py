...
# NLP imports with deployment optimization
try:
    import spacy
    ...
    try:
        ...
        st.warning("⏳ Downloading spaCy English model for first-time setup...")
        try:
            import subprocess
            import sys
            ...
            nlp = None
            st.error(f"❌ Could not download spaCy model: {str(e)}")
            st.info("💡 The app will run with limited NLP features.")
except ImportError:
...
# DEPLOYMENT OPTIMIZATION: Disable Transformers for now to prevent memory issues
TRANSFORMERS_AVAILABLE = False
SENTIMENT_AVAILABLE = False
sentiment_pipeline = None

# Commented out to prevent memory overload in deployment
# try:
...
<div class="dashboard-header">
    ...
    <p style="font-size: 0.9rem; opacity: 0.8;">⚡ Optimized for fast deployment performance</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar.expander("🔧 System Status"):
    statuses = [
        ...
        ("Basic Sentiment", True),  # Always available via TextBlob
    ...
    with col1:
        with st.container():
            ...
            <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 10px; border-left: 4px solid #28a745; margin-bottom: 1rem;">
                <h4 style="color: #28a745; margin: 0 0 0.5rem 0;">🧠 Smart NLP Analysis</h4>
                <p style="color: #6c757d; margin: 0; font-size: 0.9rem;">
                    Powered by spaCy, scikit-learn, and TextBlob for fast, intelligent semantic understanding.
                </p>
            </div>
            """, unsafe_allow_html=True)
...
<div style="text-align: center; padding: 1rem; color: #6c757d;">
    ...
    <p>Powered by spaCy • scikit-learn • TextBlob • Streamlit</p>
    <p style="font-size: 0.8rem; opacity: 0.7;">⚡ Deployment-optimized for fast performance and reliability</p>
</div>
""", unsafe_allow_html=True)